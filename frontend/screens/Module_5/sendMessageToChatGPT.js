...
const Chat = ({ navigation }) => {
    ...
    const [recommend1Message, setRecommend1Message] = useState('What will be my spending like for next month?');
    const [recommend2Message, setRecommend2Message] = useState('Suggest me ways to reduce expenses based on my data.');
    const [recommend3Message, setRecommend3Message] = useState('What income tax advice can you give me as a father?');
    ...
    const makeResponse = async (user_message) => {
        const response = await axios.post(`http://${Url}:8000/vhack_app/chatbot/send_message`, {
            thread_id: thread,
        ...
        if (response.data.forecast_visualisation_response !== null) {
            // const unformattedData =
            console.log(response.data.forecast_visualisation_response);

            preData = response.data.forecast_visualisation_response;
        }

        console.log(preData.type, preData.data);
                ...
                >
                    ...
                    {chatInit &&
                        chatInitMessage.map((msg, index) => (
                            <View>
                                <View style={[styles.messageBubble, msg.isSender ? styles.sender : styles.receiver]}>
                                    <Text style={[styles.messageText, { marginBottom: sh(10) }]}>{msg.text}</Text>
                                    ...
                                    <TouchableOpacity onPress={selectRecommend1}>
                                        ...
                                        >
                                            <Text style={[styles.messageText, { color: '#9F97F7' }]}>
                                                {recommend1Message}
                                            </Text>
                                        </View>
                                    </TouchableOpacity>
