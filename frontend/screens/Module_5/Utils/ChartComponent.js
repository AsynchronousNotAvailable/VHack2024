...
const ChartComponent = ({ jsonData }) => {
    ...
    switch (type) {
                ...
                <LineChart
                    data={{
                        ...
                        datasets: [
                            {
                                data: data.y,
                                color: (opacity = 1) => `rgba(100, 100, 219,${opacity})`,
                            },
                        ],
                    }}
                    width={sw(300)}
        ...
        case 'bar':
            ...
            break;
        case 'pie':
            const pieColors = ['#3EDAD8', '#48D6FD', '#9ADCFF', '#9BB7D9', '#9197C8'];
            ChartToRender = (
                <PieChart
        ...
        case 'forecast':
            const n = 5;
            const actualLabels = data.actualdata.x.map((label, index) => (index % n === 0 ? label : ''));
            const forecastLabels = data.forecastdata.x.map((label, index) => (index % n === 0 ? label : ''));
            ...
            ChartToRender = (
                <LineChart
                    data={{
                        ...
                        datasets: [
                            {
                                data: [...data.actualdata.y, ...Array(data.forecastdata.y.length).fill(null)],
                                color: (opacity = 1) => `rgba(100, 100, 219,${opacity})`,
                                strokeWidth: 4,
                                withDots: false,
                            },
                            {
                                data: [...Array(data.actualdata.y.length).fill(null), ...data.forecastdata.y],
