    ...
    useFocusEffect(
        useCallback(() => {
            const fetchAppointmentsData = async () => {
                try {
                    const userId = 1;
                    const response = await axios.get(`http://${Url}:3000/appointments/${userId}`);
                    ...
                    const formattedAppointments = fetchedAppointments.map((appointment) => {
                        ...
                        return {
                            ...consultant,
                            profileImage: advisors.filter((advisor) => advisor.id === consultant.id)[0].profileImage,
                            date: formattedDate,
                            time: formattedTime,
                        };
                    });

                    setSessions(formattedAppointments);
            ...
            fetchAppointmentsData(); // Call the fetchAppointmentsData function
        ...
        <ScrollView contentContainerStyle={styles.scrollViewContainer}>
            ...
            >
                ...
                <View style={styles.textContainer}>
                    ...
                    <TouchableOpacity
                        style={styles.button}
                        onPress={() => navigation.navigate('Consult_Match', { advisors: advisors })}
                    >
                        ...
                        >
                            <View style={styles.advisorRow}>
                                <View style={styles.leftContent}>
                                    <Image
                                        source={session.profileImage}
                                        style={styles.sessionProfileImage}
                                    />
                                </View>
